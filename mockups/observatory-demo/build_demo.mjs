...
import { castMeetings } from "./demo_calendar.mjs";
...
(0, eval)(obsSrc + ";globalThis.PEOPLE=PEOPLE;globalThis.DUST=DUST;globalThis.MEETINGS=MEETINGS;globalThis.YEAR0=YEAR0;globalThis.YEARS=YEARS;globalThis.WEEK_START=WEEK_START;globalThis.REGIONS=REGIONS;");
...
// calendar: every meeting cast onto stars of the demo sky (see demo_calendar.mjs)
const V = new Map(JSON.parse(fs.readFileSync(path.join(REAL_CAP, "vectors.json"), "utf8")).map(v => [v.id, v.vector]));
const normKey = (n) => n.toLowerCase().replace(/\(.*?\)/g, "").replace(/[^a-z\s.'-]/g, "").replace(/\b(phd|edd|med|mba|dr|jr|sr|ii|iii|he|him|his|she|her)\b\.?/g, "").replace(/\s+/g, " ").trim();
const { pipeline } = await import("@xenova/transformers");
const fe = await pipeline("feature-extraction", "Xenova/all-MiniLM-L6-v2", { quantized: true });
const embed = async (texts) => { const t = await fe(texts, { pooling: "mean", normalize: true }); return texts.map((_, i) => Array.from(t.data.slice(i * 384, (i + 1) * 384))); };
const demoMeetings = await castMeetings({ MEETINGS, PEOPLE, frags, V, norm: normKey, mapName, swapCal, embed });
console.log("calendar:", demoMeetings.length, "meetings ·", demoMeetings.filter(m => m.ppl.length).length, "with stars");
